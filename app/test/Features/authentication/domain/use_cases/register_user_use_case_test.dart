...
main() {
  ...
  test('Should register a user calling once the Register User Repository.', () async {
    useCase(userMock);
    expect(verify(repository(captureAny)).captured.single, userMock);
  });

  test('Should throw a authentication exception when its not possible use repository', () async {
    when(repository(userMock)).thenThrow(Exception());
    var response = await useCase(userMock);
    expect(response.fold((l) => l, (r) => null), isInstanceOf<AuthenticationException>());
  });
}
